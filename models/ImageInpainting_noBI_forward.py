...
class ImageInpainting:
    ...
    def __init__(self, cuda, device):
        ...
        IG.load_state_dict(torch.load(os.path.join(dirname, "model_saves/G_GatedUNet_UNet_noBI.pt"), map_location=device))
        IG.to(device)
        IG.eval()
        self.IG = IG
    ...
    def forward(self, crop, mask):
        ...
        IG_out = self.IG(crop*(1-mask), condition=torch.cat((mask, noise), dim=1), tanh=True)
