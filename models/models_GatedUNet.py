...
class GatedUNet(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, width_unit, crop_dim, blocks=1, leaky=False, dilation=False, 
        ...
        self.tail = nn.Sequential(
            GatedConv2d(in_channels, width_unit, leaky=leaky, batch_norm=batch_norm, sn=sn, **kwargs),
            ResBlock(width_unit, blocks=blocks, leaky=leaky, sn=sn, **kwargs))
        ...
        if dilation:
            self.floor = nn.Sequential(GatedConv2d(width_unit*8, width_unit*8, leaky=leaky, batch_norm=batch_norm, sn=sn,
                                                   kernel_size=3, dilation=2, padding=2, padding_mode="reflect"),
                                       GatedConv2d(width_unit*8, width_unit*8, leaky=leaky, batch_norm=batch_norm, sn=sn,
        ...
        self.head = nn.Sequential(GatedConv2d(width_unit*2, width_unit, leaky=leaky, batch_norm=batch_norm, sn=sn, **kwargs),
    ...
    def forward(self, x, condition=None, sigmoid=False, tanh=False):
        ...
        out = self.downblock3(skip3)                           # [bs, width_unit*8, crop_dim/8, crop_dim/8], RF: 17+14=31
...
class UNetBlock(nn.Module):
    ...
    The sampling parameter determines whether to upsample or downsample the
    ...
    def __init__(self, in_channels, out_channels, sampling, blocks=1, leaky=False, batch_norm=True, sn=False, **kwargs):
        ...
        if sampling == "down":
            self.block = nn.Sequential(
                ...
                GatedConv2d(in_channels, out_channels, batch_norm=batch_norm, leaky=leaky, sn=sn, **kwargs),
                ResBlock(out_channels, blocks=blocks, leaky=leaky, batch_norm=batch_norm, sn=sn, **kwargs))
        else:
            self.block = nn.Sequential(
                GatedConv2d(in_channels, out_channels*2, batch_norm=batch_norm, leaky=leaky, sn=sn, **kwargs), 
...
class ResBlock(nn.Module):
    """ResBlock with optional BatchNorm2d and LeakyReLU instead of ReLU."""
    ...
    def __init__(self, channels, blocks=1, leaky=False, batch_norm=True, sn=False, **kwargs):
        ...
        for _ in range(blocks - 1):
            layers.append(GatedConv2d(channels, channels, batch_norm=batch_norm, leaky=leaky, sn=sn, **kwargs))

        self.block = nn.Sequential(*layers)
        self.head = GatedConv2d(channels, channels, batch_norm=False, leaky=None, sn=sn, **kwargs)
        if batch_norm:
...
class GatedConv2d(nn.Module):
    """Gated Convolution layer with activation, optional BatchNorm2d and LeakyReLU instead of ReLU."""
    def __init__(self, in_channels, out_channels, batch_norm=True, leaky=False, sn=False, **kwargs):
        ...
        self.mask_conv2d = nn.Conv2d(in_channels, out_channels, **kwargs)
        if sn:
    ...
    def forward(self, x):
        ...
        gating = self.mask_conv2d(x)
        if self.batch_norm:
            feature = self.batch_norm2d(feature)
        if self.leaky is not None:
            return self.activation(feature) * self.sigmoid(gating)
        else:
            return feature * self.sigmoid(gating)             
