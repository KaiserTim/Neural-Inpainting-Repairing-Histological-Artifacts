# modified from https://github.com/tigvarts/vaeac/blob/master/VAEAC.py
...
from models.vaeac.prob_utils import normal_parse_params
...
class VAEAC(Module):
    ...
    def make_latent_distributions(self, src, tgt, loc_mask, pad_mask, no_proposal=False):
        """
        ...
        if no_proposal:
            ...
            proposal_params = self.proposal_network(torch.cat((tgt, loc_mask, pad_mask), 1))
            proposal = normal_parse_params(proposal_params, 1e-3)
        prior_params = self.prior_network(torch.cat((src, loc_mask, pad_mask), 1))
    ...
    def batch_vlb(self, src, tgt, loc_mask, pad_mask, beta=0, device=None):
        """
        Compute differentiable lower bound for the given batch of objects
        and mask.
        """
        proposal, prior = self.make_latent_distributions(src, tgt, loc_mask, pad_mask)
        prior_regularization = self.prior_regularization(prior)
        latent = proposal.rsample()
        rec_params = self.generative_network(latent)
#         rec_loss = -self.l1_loss(tgt, rec_params)
        rec_loss = self.rec_log_prob(tgt, rec_params, loc_mask*(~pad_mask))
        kl = kl_divergence(proposal, prior).view(tgt.shape[0], -1).sum(-1)
        if beta != 0:
            latent_size = prior.mean.shape
            if device is not None:
                std_normal = torch.distributions.Normal(torch.zeros(latent_size).cuda(device), torch.ones(latent_size).cuda(device))
            else:
                std_normal = torch.distributions.Normal(torch.zeros(latent_size), torch.ones(latent_size))
            kl_reg = kl_divergence(prior, std_normal).view(tgt.shape[0], -1).sum(-1)
        else:
            kl_reg = 0
        return rec_loss - kl + prior_regularization - beta*kl_reg
    ...
    def generate_samples_params(self, src, loc_mask, pad_mask=None, K=1):
        """
        ...
        """
        _, prior = self.make_latent_distributions(src, None, loc_mask, pad_mask, no_proposal=True)
        samples_params = []
        for i in range(K):
